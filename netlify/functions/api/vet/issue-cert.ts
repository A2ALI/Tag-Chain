...
export async function handler(event: any, context: any) {
  ...
  try {
    ...
    const { animal_id, certificate_type, expires_at, document_url } = JSON.parse(event.body);
    ...
    const validTypes = ['VET_HEALTH', 'EXPORT', 'HALAL', 'OTHER'];
    if (!validTypes.includes(certificate_type)) {
      return {
        statusCode: 400,
        body: JSON.stringify({ error: 'Invalid certificate type' })
      };
    }
    ...
    const { data: vetUser, error: userError } = await supabase
      .from('users')
      .select('id, role')
      .eq('id', user.id)
      .single();

    if (userError || !vetUser || vetUser.role !== 'vet') {
      return {
        statusCode: 403,
        body: JSON.stringify({ error: 'User is not authorized as a veterinarian' })
      };
    }
