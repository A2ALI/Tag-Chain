    ...
    >
      ...
      <div className="grid gap-1.5">
        {payload.map((item: any, index: number) => {
          ...
          const itemConfig = getPayloadConfigFromPayload(config, item, key);
          const indicatorColor = color || (item.payload as ChartPayload)?.fill || item.color;
...
function ChartLegendContent({
  ...
  if (!payload || !(payload as ChartPayload[])?.length) {
    return null;
  }
  ...
  return (
    ...
    >
      {Array.isArray(payload) && (payload as ChartPayload[]).map((item: ChartPayload) => {
        const key = `${nameKey || (item.dataKey as string) || "value"}`;
        const itemConfig = getPayloadConfigFromPayload(config, item, key);

        return (
          <div
            key={item.value ? item.value.toString() : key}
            className={cn(
